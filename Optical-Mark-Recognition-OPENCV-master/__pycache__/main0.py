...
cap.set(10,160)## رقم الخاصية بتاعة السطوع بنعبر عنه ب10 وبندي قيمه السطوع للصورة تكون 160
heightImg = 700
widthImg  = 700
questions=5
choices=5
...
while True:
    ...
    imgFinal = img.copy()##نسخة من الصورة الأصلية عشان نعرض النتيجة عليها.
    imgBlank = np.zeros((heightImg,widthImg, 3), np.uint8) # CREATE A BLANK IMAGE FOR TESTING DEBUGGING IF REQUIRED
    imgGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # CONVERT IMAGE TO GRAY SCALE
    imgBlur = cv2.GaussianBlur(imgGray, (5, 5), 1) # ADD GAUSSIAN BLUR--(5, 5) → حجم الفلتر (Kernel size)---هنا القيمة 1 → تنعيم خفيف
    imgCanny = cv2.Canny(imgBlur,10,70) # APPLY CANNY 10 = الحد الأدنى (low threshold).--70 = الحد الأعلى (high threshold)

    try:
        ## FIND ALL COUNTOURS
        imgContours = img.copy() # COPY IMAGE FOR DISPLAY PURPOSES بيعمل نسخة من الصورة الأصلية عشان نرسم عليها الكونتورز (الحدود)
        imgBigContour = img.copy() # COPY IMAGE FOR DISPLAY PURPOSES نسخة تانية من الصورة، مخصوص لعرض أكبر كونتور
        contours, hierarchy = cv2.findContours(imgCanny, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) # FIND ALL CONTOURS --cv2.CHAIN_APPROX_NONE: يعني خزّن كل نقاط الكونتور بدون تبسيط
        ## hierarchy: دي مصفوفة إضافية بتوضح العلاقة بين الكونتورز وبعضها مين جوه مين وكده
        cv2.drawContours(imgContours, contours, -1, (0, 255, 0), 10) # DRAW ALL DETECTED CONTOURS
        rectCon = utlis.rectContour(contours) # FILTER FOR RECTANGLE CONTOURS--عشان نصفي الكونتورز ونختار بس اللي شكلها مستطيل
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            pts1 = np.float32(biggestPoints) # النقاط الأصلية (زوايا المستطيل)
            pts2 = np.float32([[0, 0],[widthImg, 0], [0, heightImg],[widthImg, heightImg]]) # الشكل المثالي اللي عايزين نوصل له (مستطيل مستقيم بحجم 700×700).
